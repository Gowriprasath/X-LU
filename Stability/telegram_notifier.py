"""
Async Telegram notification gateway.
"""
...
def _send(message: str) -> None:
    ...
    try:
        ...
        print(f"[Telegram] Send failed: {exception}")
...
if __name__ == "__main__":
    ...
    sent_messages = []
    tn._ENABLED = True
    tn._send = lambda msg: sent_messages.append(msg)
    ...
    send_recovery_alert()
    send_pnl_update({"ticket": 1, "direction": "BUY", "session": "NY"}, 1.0)
    send_sl_update(1, 2340.0, "BE")
    send_partial_close(1, 0.01, 0.01)
    send_session_summary({"session": "NY", "trades_taken": 1})
    send_wisdom_updated(1, 0, "test")
    send_postmortem({"date": "today"})
    send_retrain_started("manual", 100)
    send_retrain_completed(50.0, 55.0, 120)
    send_startup_alert("XAUUSD", "claude", "AUTO")
    send_halt_alert("test")
    send_mt5_alert("STALE", "test")
    time.sleep(0.2)
